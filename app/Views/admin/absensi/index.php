...
<div x-data='absensiKerja(<?= htmlspecialchars(json_encode($kerjaJson), ENT_QUOTES) ?>, <?= htmlspecialchars(json_encode($guruJson), ENT_QUOTES) ?>)'
    ...
    <div class="flex items-start justify-between gap-3 mb-1">
        ...
        <?php if (! ($hariAktif && $total > 0)): ?>
            ...
            <div x-data="{ waOpen:false, waText:'', waNomor:'' }" class="shrink-0">
                <button type="button" @click="waText = buildAbsensiPesan(); waOpen = true"
                        class="inline-flex items-center gap-1.5 rounded-xl bg-green-600 hover:bg-green-700 text-white font-bold px-3.5 py-2 text-xs transition">
                ...
                <div x-show="waOpen" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
                        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                            <h3 class="font-bold text-slate-800">Bagikan Kehadiran ke WhatsApp</h3>
                        ...
                        <div class="p-5 space-y-3">
                            <p class="text-xs text-slate-500">Teks dibuat dari status yang sedang tampil. <b>Silakan revisi</b> dulu bila perlu. Kosongkan nomor untuk memilih kontak/grup langsung di WhatsApp.</p>
                            <div>
                            ...
                            <div>
                                ...
                                <textarea x-model="waText" rows="10" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-brand-500 leading-relaxed"></textarea>
                            </div>
                        </div>
...
<script>
    ...
    function buildAbsensiPesan() {
        ...
        // Kehadiran kerja (di luar jadwal) — baca dari panel (status live).
        const stKerja = { hadir: 'Hadir', telat: 'Telat', izin: 'Izin', sakit: 'Sakit', alpa: 'Tidak Hadir' };
        const kerjaRows = document.querySelectorAll('[data-kerja-nama]');
        if (kerjaRows.length > 0) {
            msg += '*Kehadiran Kerja (di luar jadwal):*\n';
            let kn = 1;
            kerjaRows.forEach(function (r) {
                const nama = r.getAttribute('data-kerja-nama') || '-';
                const st   = r.getAttribute('data-kerja-status') || 'hadir';
                const jm   = r.getAttribute('data-kerja-jm') || '';
                const ket  = r.getAttribute('data-kerja-ket') || '';
                let line = kn + '. @' + nama + ' — ' + (stKerja[st] || st);
                if (st === 'telat' && jm) line += ' (masuk ' + jm + ')';
                msg += line + '\n';
                if (ket) msg += '   _' + ket + '_\n';
                kn++;
            });
