...
<div x-data="guruPage()">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-5">
        <div class="flex flex-col lg:flex-row lg:items-center gap-3">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="<?= site_url('admin/master/guru/export') ?>" title="Keluarkan semua data guru ke file Excel" class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2.5 transition">
        ...
        <form method="post" action="<?= site_url('admin/master/guru/bulk-delete') ?>" x-ref="bulkForm" class="hidden">
            <?= csrf_field() ?>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <?php if ($pager): ?>
            <div class="px-6 py-4 border-t border-slate-100">
                <?= $pager->only(['q', 'status'])->links('default', 'admin') ?>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div x-show="importOpen" x-cloak x-transition.opacity.duration.200ms class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <form method="post" action="<?= site_url('admin/master/guru/import-preview') ?>" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <p class="text-sm text-slate-500 mb-3"><b>Import = memasukkan data.</b> Unggah file Excel sesuai template. Setelah diunggah, data akan ditampilkan dalam <b>pratinjau yang bisa diedit langsung</b> sebelum benar-benar disimpan. Baris dengan <b>Kode Guru</b> yang sudah ada akan diperbarui (tidak dobel).</p>
...
function guruPage() {
    return {
        ...
        submitBulk(mode) {
            ...
            if (mode === 'all') {
                if (!confirm('HAPUS SEMUA data guru? Tindakan ini menghapus seluruh daftar dan tidak dapat dibatalkan.')) return;
