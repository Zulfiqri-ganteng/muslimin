...
<?php else: ?>
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-3">
            <h2 class="font-bold text-slate-800"><?= esc($guru['nama']) ?></h2>
            <a href="<?= site_url('jadwal-guru/' . $guruId . '/pdf') ?>" target="_blank" class="inline-flex items-center gap-1.5 rounded-lg bg-red-600 hover:bg-red-700 text-white text-xs font-semibold px-3 py-2 transition">
                    ...
                    <?php foreach ($jam as $j): ?>
                        <tr class="hover:bg-slate-50/50">
                            <td class="px-3 py-2 border-b border-slate-100 whitespace-nowrap align-top">
                                <span class="font-bold text-brand-700"><?= ucfirst($j['shift']) ?> Jam <?= esc($j['jam_ke']) ?></span><br>
                                ...
                                <td class="p-1.5 border-b border-l border-slate-100 align-top <?= $td ? 'bg-brand-50/60' : '' ?>">
                                    <?php if ($c): ?>
                                        <div class="rounded-lg bg-emerald-50 border border-emerald-100 p-1.5">
                                            <p class="font-semibold text-emerald-700 text-sm leading-tight"><?= esc($c['nama_kelas']) ?></p>
                                            <p class="text-[11px] text-slate-500"><?= esc($c['kode_mapel']) ?></p>
                                        </div>
                                    <?php endif; ?>
                                </td>
                            <?php endforeach; ?>
